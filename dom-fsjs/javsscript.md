...
).innerHTML = 'Bhaskar Sahu';

document.querySelector('.side-bar .crayons-card .color-base-70').innerHTML =
  'Coding, Coffee, and Chill';
```

1. Website Name: [Apple](https://support.apple.com/en-in)
...
searchBor.addEventListener('mouseover', () => {
...
```javascript
document.querySelector('.display-heading-1').style.cssText =
...
btn.addEventListener('mouseover', () => {
  document.querySelector(
    '[data-test-label=landing-big-cta] .login-btn-text'
...
```javascript
document.querySelector('.product-card-content .wide .desc').style.color =
  'orange';
```
